...
public class CmtyServiceManaer {
	...
	public List<ActionDef> getToolActionDefs(String targetId) {
		if(targetId == null || targetId.trim().equals("") || toolDefProviders.isEmpty()) {
			return null;
		}
		...
		for(IToolActionDefProvider p : toolDefProviders.values()) {
			...
			for(ActionDef ad : as) {
				if(ad.getUrl() != null && ad.getUrl().equals(targetId)) {
					acts.add(ad);
				}
			}
		}
	...
	public List<IMenuDefProvider> getMenuDefProvider(String targetId) {
		...
		for(IMenuDefProvider p : this.menuDefProviders.values()) {
			if(p.getTargetId().equals(targetId)) {
				ps.add(p);
			}
		}
		return ps;
		...
		actionTracker = new ServiceTracker(context, IAction.class.getName(), null) {
		 ...
		 panelTracker = new ServiceTracker(context, IPanelProvider.class.getName(), null) {
		      ...
		      public Object addingService(ServiceReference reference) {
		    	  ...
		    	  if(panelProviders.containsKey(act.getTargetId())) {
		    		  IPanelProvider existPanel = panelProviders.get(act.getTargetId());
		    		  throw new CommonException("ExistRepeatPanel",
		    				  existPanel.getId(),act.getId());
		    	  }
		    	  panelProviders.put(act.getTargetId(), act);
		        return act;
		      }
		 };
		 panelTracker.open();
